...
export default function FacultyChaptersScreen({ route }) {
  ...
  const fetchChapters = async () => {
    try {
      ...
      const res = await fetch(
        `/api/faculty/chapters?subjectId=${subjectId}&classAssigned=${grade}&section=${section}`
      );
  ...
  const handleUploadExcel = async () => {
    try {
      ...
      setLoading(true);
      const res = await fetch(`/api/faculty/chapters/upload-excel`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleUploadPDF = async (chapterId) => {
    ...
    try {
      ...
      setLoading(true);
      const res = await fetch(`/api/faculty/chapters/${chapterId}/upload-pdf`, {
        method: "POST",
        body: formData,
      });
    ...
    Alert.alert("Confirm", "Delete this chapter?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            setLoading(true);
            const res = await fetch(`/api/faculty/chapters/${id}`, {
              method: "DELETE",
            });
            const text = await res.text();
  ...
  const handleUpdate = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/faculty/chapters/${selectedChapter._id}`, {
        method: "PUT",
